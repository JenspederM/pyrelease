...
def get_latest_tag(path: str = ".") -> str:
    """Get the latest git tag in the repository at the given path.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create a changelog of commits since a specific commit."
    )
    parser.add_argument(
        "--path",
        type=str,
...
def main():
    ...
    if is_git_repo(args.path):
        create_changelog_since_commit(commit_id, args.path)
    else:
        print(f"The path '{args.path}' is not a git repository.")
